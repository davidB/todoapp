...
9. **`@name`/`#tag` title syntax** (`FR-32`/`FR-33`): should `@me` be a reserved alias (vs. a literal actor id, same as any other name)? Is the trigger character set (`[A-Za-z0-9_-]`, shared by both) final? `@name` and `#tag` now share one scan (`extract_title_syntax` in `todoapp-core`) rather than parsing the title twice — a future title syntax should extend that same function (one more trigger-char branch in the loop) instead of adding a parallel scanner, so order/precedence stays implicit in a single left-to-right pass.
