...
9. **`@name`/`#tag`/`[...]` title syntax** (`FR-32`/`FR-33`/`FR-34`): should `@me` be a reserved alias (vs. a literal actor id, same as any other name)? Is the trigger character set (`[A-Za-z0-9_-]`, shared by the first two) final? Is the `[...]` recurrence grammar (`daily`/`every N days`/`every <weekdays>`/`monthly`/`every N months`) sufficient, or should it grow toward a fuller RRULE-like syntax? All three now share one scan (`extract_title_syntax` in `todoapp-core`) rather than parsing the title multiple times — a future title syntax should extend that same function instead of adding a parallel scanner, so order/precedence stays implicit in a single left-to-right pass.
